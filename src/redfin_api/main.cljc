(ns redfin-api.main
  "Kotodama WASM entrypoint for the Redfin_api clean-room actor (L4) — Clojure port.
  ...
  facts (`redfin_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "redfin_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "Listing"     :plural "listings"     :id-prefix "redfinap_lis"
    :fields [:propertyId :price :status :listedAt]         :required [:price :status]
    :coerce {:price :float}                :refs {:propertyId "Property"}}
   {:entity "Property"    :plural "properties"   :id-prefix "redfinap_pro"
    :fields [:address :beds :baths :sqft :lat]             :required [:address :beds]
    :coerce {:beds :int :baths :float :sqft :int :lat :float} :refs {}}
   {:entity "Agent"       :plural "agents"       :id-prefix "redfinap_age"
    :fields [:name :license :brokerage]                    :required [:name :license]
    :coerce {}                             :refs {}}
   {:entity "Transaction" :plural "transactions" :id-prefix "redfinap_tra"
    :fields [:listingId :buyerId :salePrice :closedAt]     :required [:salePrice :closedAt]
    :coerce {:salePrice :float}            :refs {:listingId "Listing"}}
   {:entity "Lease"       :plural "leases"       :id-prefix "redfinap_lea"
    :fields [:propertyId :tenantId :rent :startsAt]        :required [:rent :startsAt]
    :coerce {:rent :float}                 :refs {:propertyId "Property" :tenantId "Tenant"}}
   {:entity "Tenant"      :plural "tenants"      :id-prefix "redfinap_ten"
    :fields [:name :email :phone]                          :required [:name :email]
    :coerce {}                             :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"redfin_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "redfin_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :redfin_api-compat/ready)
